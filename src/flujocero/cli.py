...
def capacidad() -> None:
    ...
    for etiqueta, tasa in [
        ...
        r = ticket_maximo_uf(
            D(str(renta)), otras, tasa, int(p.d("financiamiento.plazo_anios")),
            p.d("financiamiento.ltv_con_fogaes"), p.d("macro.valor_uf_clp"),
...
def demo() -> None:
    ...
    us = [
        Unidad("SM-1D-35", D(2600), D(35), "1D1B", "san-miguel", "san-miguel/gran-avenida",
               D("8.6"), 24, True, riesgo_microzona=D("0.2"), catalizador=D("0.3")),
        Unidad("NU-2D-55", D(4900), D(55), "2D2B", "nunoa", "nunoa/irarrazaval",
               D("16.5"), 31, True, riesgo_microzona=D("0.6"), catalizador=D("0.1")),
        Unidad("LF-2D-50", D(3700), D(50), "2D1B", "la-florida", "la-florida/rojas-magallanes",
               D("12.5"), 18, True, riesgo_microzona=D("0.7"), catalizador=D("0.1")),
        Unidad("CO-1D-40", D(2761), D(40), "1D1B", "concepcion", "concepcion/centro",
               D("10.5"), 12, True, riesgo_microzona=D("0.4"), catalizador=D("0.0")),
        Unidad("EC-SAT", D(2112), D(32), "1D1B", "estacion-central", "estacion-central/santa-isabel",
               D("7.3"), 40, True, microzona_saturada=True),
    ]
    ...
    typer.echo(hdr); typer.echo("-" * len(hdr))
...
def gates() -> None:
    ...
    if inv.crudo("estrategia_dfl2")["objetivo_unidades"] > p.crudo("tributacion.limite_dfl2_por_persona_natural")["v"]:
